...
func FindTopTags(limit int) []searchAPI.Tag {
	...
	results, error := internalDB.Query(`
		SELECT t.name, count(t.name)
		       ...
		       GROUP BY f.name`)
	if error != nil {
		logger.Logf(logger.LogLevelError, "Cant search top tags %s", error.Error())
	}
	for results.Next() {
		columns, err := results.Columns()
		if err != nil {
			logger.Logf(logger.LogLevelError, "Cant get columns for top tags result %s", error.Error())
		}
